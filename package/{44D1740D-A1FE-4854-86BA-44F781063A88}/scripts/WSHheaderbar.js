...
oHeaderBar = function () {
	...
	this.columns = [];
	this.borders = [];
	this.totalColumns = window.GetProperty("SYSTEM.HeaderBar.TotalColumns", 14);
	...
	this.initColumns = function () {
		...
		if (this.totalColumns == 0) {
			...
			for (var i = 0; i < 7; i++) {
				switch (i) {
				case 0:
					tmp = window.GetProperty("SYSTEM.HeaderBar.label", "Cover^^State^^Index^^#^^Title^^Year^^Artist^^Album^^Genre^^Mood^^Rating^^Plays^^Bitrate^^Time");
					break;
				case 1:
					tmp = window.GetProperty("SYSTEM.HeaderBar.tf", "null^^null^^$num(%list_index%,$len(%list_total%))^^$if2(%tracknumber%,-)^^$if2(%title%,%filename_ext%)^^$if(%date%,$year($replace(%date%,/,-,.,-)),'-')^^$if(%length%,%artist%,'Stream')^^$if2(%album%,$if(%length%,'Single','Web radios'))^^$if2(%genre%,'Other')^^$rgb(255,120,170)$if(%mood%,1,0)^^$rgb(255,255,50)$if2(%rating%,0)^^$if2(%play_counter%,$if2(%play_count%,0))^^$if(%__bitrate_dynamic%, $if(%isplaying%,$select($add($mod(%_time_elapsed_seconds%,2),1),%__bitrate_dynamic%,%__bitrate_dynamic%)'K',$if($stricmp($left(%codec_profile%,3),'VBR'),%codec_profile%,%__bitrate%'K')),' '$if($stricmp($left(%codec_profile%,3),'VBR'),%codec_profile%,%__bitrate%'K'))^^$if(%isplaying%,$if(%length%,-%playback_time_remaining%,'0:00'),$if2(%length%,'00:00'))");
					break;
				case 2:
					tmp = window.GetProperty("SYSTEM.HeaderBar.tf2", "null^^null^^null^^null^^$if(%length%,%artist%,)^^null^^null^^$if2(%genre%,'Other')^^null^^null^^null^^null^^null^^%__bitrate% kbps");
					break;
				case 3:
					tmp = window.GetProperty("SYSTEM.HeaderBar.percent", "0^^5000^^0^^5000^^70000^^0^^0^^0^^0^^0^^10000^^0^^0^^10000");
					break;
				case 4:
					tmp = window.GetProperty("SYSTEM.HeaderBar.ref", "Cover^^State^^Index^^Tracknumber^^Title^^Date^^Artist^^Album^^Genre^^Mood^^Rating^^Playcount^^Bitrate^^Duration");
					break;
				case 5:
					tmp = window.GetProperty("SYSTEM.HeaderBar.align", "0^^1^^1^^2^^0^^2^^0^^0^^0^^1^^1^^2^^1^^2");
					break;
				case 6:
					tmp = window.GetProperty("SYSTEM.HeaderBar.sort", "%album artist% | $if(%album%,%date%,'9999') | %album% | %discnumber% | %tracknumber% | %title%^^%album artist% | $if(%album%,%date%,'9999') | %album% | %discnumber% | %tracknumber% | %title%^^null^^%tracknumber% | %album artist% | $if(%album%,%date%,'9999') | %album% | %discnumber% | %title%^^%title% | %album artist% | $if(%album%,%date%,'9999') | %album% | %discnumber% | %tracknumber%^^%date% | %album artist% | %album% | %discnumber% | %tracknumber% | %title%^^%artist% | $if(%album%,%date%,'9999') | %album% | %discnumber% | %tracknumber% | %title%^^$if2(%album%,%artist%) | $if(%album%,%date%,'9999') | %album artist% | %discnumber% | %tracknumber% | %title%^^%genre% | %album artist% | $if(%album%,%date%,'9999') | %album% | %discnumber% | %tracknumber% | %title%^^%mood% | %album artist% | $if(%album%,%date%,'9999') | %album% | %discnumber% | %tracknumber% | %title%^^%rating% | %album artist% | $if(%album%,%date%,'9999') | %album% | %discnumber% | %tracknumber% | %title%^^$if2(%play_counter%,$if2(%play_count%,0)) | %album artist% | $if(%album%,%date%,'9999') | %album% | %discnumber% | %tracknumber% | %title%^^%__bitrate% | %album artist% | $if(%album%,%date%,'9999') | %album% | %discnumber% | %tracknumber% | %title%^^$if2(%length%,' 0:00') | %album artist% | $if(%album%,%date%,'9999') | %album% | %discnumber% | %tracknumber% | %title%");
					break;
				};
