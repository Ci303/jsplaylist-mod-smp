...
oGroup = function (
  ...
  if (properties.autocollapse) {
    if (
      focusedTrackId >= this.start &&
      focusedTrackId < this.start + this.count
    ) {
      // focused track is in this group!
...
oItem = function (
  ...
  this.setGroupMeta = function () {
    if (this.type == 1) {
      if (this.metadb) {
        this.l1 = fb
          .TitleFormat(p.list.groupby[cGroup.pattern_idx].l1)
          .EvalWithMetadb(this.metadb);
        this.r1 = fb
          .TitleFormat(p.list.groupby[cGroup.pattern_idx].r1)
          .EvalWithMetadb(this.metadb);
        this.l2 = fb
          .TitleFormat(p.list.groupby[cGroup.pattern_idx].l2)
          .EvalWithMetadb(this.metadb);
        this.r2 = fb
          .TitleFormat(p.list.groupby[cGroup.pattern_idx].r2)
          .EvalWithMetadb(this.metadb);
      }
    }
  };
  ...
  this.parseTF = function (tf, default_color) {
    ...
    if (pos > -1) {
      ...
      tmp = tab[1];
      result[1] = eval(
        "0xFF" + tmp.substr(4, 2) + tmp.substr(2, 2) + tmp.substr(0, 2),
      );
      while (i < fin) {
        txt = txt + tab[i + 1];
        i += 2;
  ...
  this.drawRowContents = function (gr) {
    ...
    for (var j = 0; j < fin; j++) {
      ...
      if (p.headerBar.columns[j].w > 0) {
        ...
        cw =
          Math.abs((p.headerBar.w * p.headerBar.columns[j].percent) / 100000) -
          g_z10;
        switch (p.headerBar.columns[j].ref) {
          case "State":
            if (p.headerBar.columns[j].tf == "null") {
              ...
              if (
                typeof this.state_color == "undefined" &&
                p.headerBar.columns[j].tf != "null"
              ) {
                this.state_tf = fb
                  .TitleFormat(p.headerBar.columns[j].tf)
                  .EvalWithMetadb(this.metadb);
            ...
            if (fb.IsPlaying) {
              if (plman.PlayingPlaylist == this.playlist) {
                if (this.track_index == p.list.nowplaying.PlaylistItemIndex) {
                  if (fb.isPaused) {
                    gr.DrawString(
                      String.fromCharCode(127).repeat(2),
                      g_font_pauseicon,
                      columnColor,
                      icon_x,
                      this.y,
                      icon_w,
                      cTrack.height + cTrack.parity,
                      cc_stringformat,
                    );
                  } else {
                  ...
                  if (
                    ...
                    if (this.queue_idx > 0) {
                      gr.DrawRect(
                        ...
                        this.y + g_z8 - 1,
                        queue_w + g_z6 + 1,
                        cTrack.height - g_z16 + 1,
                        1.0,
                        columnColor & 0x77ffffff,
                      );
                      gr.DrawString(
                        num(this.queue_idx, 2),
                        g_font_queue_idx,
                        columnColor,
                        icon_x + g_z3,
                        this.y,
                    ...
                    gr.FillSolidRect(
                      ...
                      this.y + g_z8 - 1,
                      queue_w + g_z6 + 2,
                      cTrack.height - g_z16 + 2,
                      g_color_normal_txt & 0x08ffffff,
                    );
                    if (this.queue_idx > 0) {
                      gr.DrawString(
                        num(this.queue_idx, 2),
                        g_font_queue_idx,
                        columnColor & 0xbbffffff,
                        icon_x + g_z3,
                        this.y,
                        queue_w,
                        cTrack.height + cTrack.parity,
              ...
              gr.SetTextRenderingHint(5);
              if (
                plman.IsPlaylistItemSelected(p.list.playlist, this.track_index)
              ) {
                // if selected
                if (this.queue_idx > 0) {
                  gr.DrawRect(
          ...
          case "Mood":
            ...
            if (g_font_guifx_found) {
              ...
              columns.mood_w =
                gr.CalcTextWidth(String.fromCharCode(252), g_font_mood) + 3;
            }
            ...
            if (this.tracktype < 2) {
              var m_color =
                this.mood != 0
                  ? this.mood_color
                  : this.text_colour_default & 0x16ffffff;
            } else {
          ...
          case "Rating":
            ...
            if (g_font_guifx_found) {
              ...
              columns.rating_w = gr.CalcTextWidth(
                String.fromCharCode(234).repeat(5),
                g_font_rating,
              );
            }
            // for minimum width for this column
            p.headerBar.columns[j].minWidth =
              columns.rating_w + zoom(7.5, g_dpi);
            //
            ...
            switch (p.headerBar.columns[j].align) {
              ...
              case 1:
                columns.rating_x =
                  cx +
                  3 +
                  Math.round((cw - 6 - one_star_w * total_stars_drawable) / 2) -
                  1;
                break;
              case 2:
                columns.rating_x =
                  cx + 3 + cw - 6 - one_star_w * total_stars_drawable;
                break;
            }
            if (g_font_guifx_found) {
              ...
              gr.DrawString(
                "b".repeat(total_stars_drawable),
                g_font_rating,
                this.text_colour_default & 0x20ffffff,
                columns.rating_x - 2,
                this.y,
                cw + 1,
                cTrack.height + cTrack.parity,
              ...
              gr.DrawString(
                "b".repeat(
                  Math.round(
                    this.rating > total_stars_drawable
                      ? total_stars_drawable
                      : this.rating,
                  ),
                ),
                g_font_rating,
              ...
              if (total_stars_drawable < 5) {
                ...
                gr.GdiDrawText(
                  "...",
                  g_font,
                  this.text_colour_default,
                  columns.rating_x - 6 + drawn_star_w,
                  this.y - 1,
                  columns.rating_w + 1,
                  cTrack.height + cTrack.parity,
                  g_LDT,
              ...
              gr.SetTextRenderingHint(3);
              gr.DrawString(
                String.fromCharCode(234).repeat(total_stars_drawable),
                g_font_rating,
                this.text_colour_default & 0x20ffffff,
                columns.rating_x - 3,
                this.y + 3,
              ...
              gr.DrawString(
                String.fromCharCode(234).repeat(
                  Math.round(
                    this.rating > total_stars_drawable
                      ? total_stars_drawable
                      : this.rating,
              ...
              if (total_stars_drawable < 5) {
                var drawn_star_w = gr.CalcTextWidth(
                  String.fromCharCode(234).repeat(total_stars_drawable),
                  g_font_rating,
                );
                gr.GdiDrawText(
          ...
          default:
            ...
            if (tf_prep != "null") {
              ...
              tf_prep = replaceAll(
                tf_prep,
                "%list_index%",
                (this.track_index + 1).toString(),
              );
              // %list_total%
              tf_prep = replaceAll(
                tf_prep,
                "%list_total%",
                p.list.count.toString(),
              );
              // %isplaying%
              if (
                fb.IsPlaying &&
                plman.PlayingPlaylist == this.playlist &&
        ...
        if (j > 0 && tf1 && tf1 != "null") {
          ...
          DrawColoredText(
            ...
            blendColors(
              this.text_colour,
              RGB(255, 255, 255),
              cList.enableExtraLine ? 0.1 : 0.0,
            ),
            cx,
            tf1_y,
            cw,
            tf1_h,
            p.headerBar.columns[j].DT_align,
          ...
          if (cList.enableExtraLine) {
            ...
            DrawColoredText(
              gr,
              tf2,
              gdi_font(g_fname, g_fsize - 1, g_fstyle),
              blendColors(this.text_colour, RGB(0, 0, 0), 0.25),
              cx,
              tf2_y,
              cw,
              tf2_h,
  ...
  this.draw = function (gr, x, y, w, h) {
    ...
    switch (this.type) {
      case 0:
            ...
            if (
              ...
              if (p.list.focusedTrackId == this.track_index) {
                ...
                gr.FillSolidRect(
                  ...
                  this.h - 6,
                  g_color_selected_bg &
                    RGBA(255, 255, 255, properties.selection_rect_alpha),
                );
                ...
                gr.DrawRect(
                  ...
                  2.0,
                  g_color_selected_bg &
                    RGBA(255, 255, 255, properties.focus_rect_alpha),
                );
              } else {
                //**
                gr.FillSolidRect(
              ...
              if (p.list.focusedTrackId == this.track_index) {
                ...
                if (properties.oddevenrowshighlight) {
                  if (properties.showgroupheaders) {
                    var parity =
                      this.track_index_in_group / 2 ==
                      Math.floor(this.track_index_in_group / 2)
                        ? 1
                        : 0;
                  } else {
                    var parity =
                      this.track_index / 2 == Math.floor(this.track_index / 2)
                        ? 1
                        : 0;
                  }
                  if (parity == 0) {
        ...
        if (
          cover.column &&
          !properties.showgroupheaders &&
          this.track_index_in_group == 0
        ) {
          gr.FillGradRect(
            this.x,
            this.y,
        ...
        if (cover.column) {
          if (
            this.row_index == 0 &&
            this.track_index_in_group > 0 &&
            this.track_index_in_group <= Math.ceil(cover.h / cTrack.height)
          ) {
          ...
          if (
            this.track_index_in_group == 0 ||
            (this.row_index == 0 && cover_draw_delta > 0)
          ) {
            // cover bg
            ...
            var groupmetadb =
              p.list.handleList[p.list.groups[this.group_index].start];
            ...
            if (typeof this.cover_img != "undefined") {
              ...
              if (this.cover_img) {
                ...
                if (p.headerBar.columns[0].w < cover.max_w) {
                  gr.DrawImage(
                    this.cover_img.Resize(cv_w, cv_h, 2),
                    cv_x,
                    cv_y,
                    cv_w,
                    cv_h,
                  ...
                  gr.DrawImage(
                    ...
                    0,
                    0,
                    this.cover_img.Width,
                    this.cover_img.Height,
                  );
                }
              ...
              gr.DrawImage(
                ...
                0,
                0,
                images.loading.Width,
                images.loading.Height,
                images.loading_angle,
                225,
              );
            }
          }
        ...
        if (!properties.enableTouchControl) {
          if (
            !cPlaylistManager.hscroll_timer &&
            mouse_x < p.playlistManager.x - p.playlistManager.woffset - 30
          ) {
            if (this.empty_row_index == 0) {
              if (dragndrop.drag_in && this.ishover && p.list.ishover) {
                if (
                  p.playlistManager.woffset == 0 ||
                  cPlaylistManager.visible
                ) {
                  if (
                    !plman.IsPlaylistItemSelected(
                      p.list.playlist,
                    ...
                    if (this.track_index > dragndrop.drag_id) {
                      gr.FillSolidRect(
                        this.x + cover.w,
                        this.y + this.h - Math.floor(cList.borderWidth / 2),
                        this.w - cover.w,
                        cList.borderWidth,
                        g_color_selected_bg,
                      );
                      gr.FillSolidRect(
                        ...
                        this.y +
                          this.h -
                          Math.floor(cList.borderWidth / 2) -
                          3 * cList.borderWidth,
                        cList.borderWidth,
                        7 * cList.borderWidth,
                        g_color_selected_bg,
                      ...
                      gr.FillSolidRect(
                        this.x + this.w - cList.borderWidth,
                        this.y +
                          this.h -
                          Math.floor(cList.borderWidth / 2) -
                          3 * cList.borderWidth,
                      ...
                      gr.FillSolidRect(
                        this.x + cover.w,
                        this.y - Math.floor(cList.borderWidth / 2),
                        this.w - cover.w,
                        cList.borderWidth,
                        g_color_selected_bg,
                      );
      ...
      case 1:
        ...
        gr.FillSolidRect(
          ...
          this.w,
          this.h - 2,
          g_color_normal_txt & 0x04ffffff,
        );

        // draw group text infos
        var text_left_padding = g_z2;
        var scrollbar_gape =
          p.scrollbar.visible && p.list.totalRows > p.list.totalRowVisible
            ? 0
            : cScrollBar.width;
        ...
        if (fb.IsPlaying && this.obj) {
          if (plman.PlayingPlaylist == this.playlist) {
            if (
              p.list.nowplaying.PlaylistItemIndex >= this.obj.start &&
              p.list.nowplaying.PlaylistItemIndex <
                this.obj.start + this.obj.count
            ) {
        ...
        this.l1_color = blendColors(
          g_color_normal_txt,
          blendColors(g_color_normal_txt, color_txt, 0.7),
          0.7,
        );
        this.l2_color = blendColors(
          g_color_normal_txt,
          blendColors(g_color_normal_bg, color_txt, 0.7),
          0.7,
        );
        ...
        switch (this.heightInRow) {
          case 1:
            var lg1_right_field_w =
              gr.CalcTextWidth(this.r1, g_font_group1) + cList.borderWidth * 2;
            gr.GdiDrawText(
              this.l1 + " / " + this.l2,
              g_font_group1,
              this.l1_color,
            ...
            gr.GdiDrawText(
              ...
              this.x + cover.w + text_left_padding,
              this.y - groupDelta - 1,
              this.w - cover.w - text_left_padding * 5 + 2 - scrollbar_gape,
              this.h,
              DT_RIGHT |
                DT_VCENTER |
                DT_CALCRECT |
            ...
            gr.FillSolidRect(
              this.x + cover.w + text_left_padding,
              Math.round(this.y + cTrack.height * 1 - groupDelta - 5),
              this.w - cover.w - text_left_padding * 5 + 2 - scrollbar_gape,
              1.0,
              line_color,
            );
          ...
          case 2:
            var lg1_right_field_w =
              gr.CalcTextWidth(this.r1, g_font_group1) + cList.borderWidth * 2;
            var lg2_right_field_w =
              gr.CalcTextWidth(this.r2, g_font_group2) + cList.borderWidth * 2;
            gr.GdiDrawText(
              this.l1,
              g_font_group1,
              this.l1_color,
            ...
            gr.GdiDrawText(
              ...
              this.l2_color,
              this.x + cover.w + text_left_padding,
              this.y + cTrack.height - groupDelta - 6,
              this.w -
                cover.w -
                text_left_padding * 4 -
                lg2_right_field_w -
            ...
            gr.FillSolidRect(
              this.x + cover.w + text_left_padding,
              this.y + cTrack.height * 2 - groupDelta - 8,
              this.w - cover.w - text_left_padding * 5 + 2 - scrollbar_gape,
              1.0,
              line_color,
            );
          ...
          default:
            ...
            gr.GdiDrawText(
              ...
              this.l2_color,
              this.x + cover.w + text_left_padding,
              this.y + cTrack.height - groupDelta - 4,
              this.w -
                cover.w -
                text_left_padding * 4 -
                lg2_right_field_w -
            ...
            gr.FillSolidRect(
              this.x + cover.w + text_left_padding,
              this.y + cTrack.height * 2 - groupDelta - 1,
              this.w - cover.w - text_left_padding * 5 + 2 - scrollbar_gape,
              1.0,
              line_color,
            );
            if (this.obj) {
              var lg3_left_field =
                this.obj.count +
                (this.obj.count > 1 ? " tracks. " : " track. ") +
                this.obj.total_group_duration_txt;
            } else {
              var lg3_left_field = "";
            }
            var lg3_right_field =
              this.group_index + 1 + " / " + p.list.groups.length;
            var lg3_right_field_w =
              gr.CalcTextWidth(lg3_right_field, g_font) + cList.borderWidth * 2;
            gr.GdiDrawText(
              lg3_left_field,
              g_font,
          ...
          for (var k = 0; k < this.obj.count; k++) {
            if (
              ...
              gr.FillSolidRect(
                ...
                this.w,
                this.h - 2,
                g_color_selected_bg & 0x20ffffff,
              );
              break;
            } else {
              ...
              if (dragndrop.drag_in && this.ishover && p.list.ishover) {
                if (
                  ...
                  if (
                    ...
                    var cover_w =
                      p.headerBar.columns[0].percent > 0
                        ? p.headerBar.columns[0].w
                        : 0;
                    if (this.track_index <= dragndrop.drag_id) {
                      ...
                      if (this.obj.collapsed) {
                        ...
                        dragndrop.drop_id =
                          this.track_index + this.obj.count - 1;
                      } else {
                        gr.FillSolidRect(
                          this.x + cover_w,
                          this.y + this.h - Math.floor(cList.borderWidth / 2),
  ...
  this.dragndrop_check = function (x, y, id) {
    var groupDelta = this.groupRowDelta * cTrack.height;
    var col_cover_w =
      p.headerBar.columns[0].percent > 0 ? p.headerBar.columns[0].w : 0;
    this.ishover =
      x >= this.x + col_cover_w &&
      x < this.x + this.w &&
      y >= this.y &&
      y < this.y + this.h - groupDelta;
    if (this.ishover) {
      // p.list.count is mandatory > 0 (if not with couldn't be in Item section)
  ...
  this.check = function (event, x, y) {
    ...
    this.rating_hover =
      this.type == 0 &&
      this.empty_row_index == 0 &&
      x >= columns.rating_x &&
      x <= columns.rating_x + columns.rating_w &&
      y > this.y + 2 &&
      y < this.y + this.h - 2;
    this.mood_hover =
      this.type == 0 &&
      this.empty_row_index == 0 &&
      x >= columns.mood_x &&
      x <= columns.mood_x + columns.mood_w - 3 &&
      y > this.y + 2 &&
      y < this.y + this.h - 2;
          ...
          if (!cTouch.down || (cTouch.down && cTouch.down_id == cTouch.up_id)) {
            ...
            if (this.type == 1) {
              ...
              if (utils.IsKeyPressed(VK_SHIFT)) {
                ...
                if (this.obj) {
                  if (
                    (properties.autocollapse && !this.obj.collapsed) ||
              ...
              p.list.metadblist_selection = plman.GetPlaylistSelectedItems(
                p.list.playlist,
              );
            } else {
              // track
                ...
                if (
                  ...
                  if (p.list.metadblist_selection.Count >= 1) {
                    ...
                    if (p.list.metadblist_selection.Count > 1) {
                      ...
                      var first_item_selected_id = p.list.handleList.Find(
                        p.list.metadblist_selection[0],
                      );
                      var last_item_selected_id = p.list.handleList.Find(
                        p.list.metadblist_selection[
                          p.list.metadblist_selection.Count - 1
                        ],
                      );
                      var contigus_count =
                        last_item_selected_id - first_item_selected_id + 1;
                    } else {
                    ...
                    if (
                      p.list.metadblist_selection.Count == 1 ||
                      (p.list.metadblist_selection.Count > 1 &&
                        p.list.metadblist_selection.Count == contigus_count)
                    ) {
            ...
            if (this.rating_hover) {
              ...
              if (this.tracktype < 2) {
                if (foo_playcount) {
                    ...
                    if (this.metadb) {
                      fb.RunContextCommandWithMetadb(
                        "Playback Statistics/Rating/" +
                          (this.l_rating == 0 ? "<not set>" : this.l_rating),
                        this.metadb,
                      );
                  ...
                  if (this.l_rating != this.rating) {
                    handles.UpdateFileInfoFromJSON(
                      JSON.stringify({ RATING: this.l_rating }),
                    );
                    this.rating = this.l_rating;
                  } else {
                    handles.UpdateFileInfoFromJSON(
              ...
              if (this.tracktype < 2) {
                ...
                if (this.l_mood != this.mood) {
                  handles.UpdateFileInfoFromJSON(
                    JSON.stringify({ MOOD: getTimestamp() }),
                  );
            ...
            if (!cList.repaint_timer) {
              ...
              cList.repaint_timer = window.SetInterval(function () {
                if (mouse_y < p.list.y + cTrack.height * 0) {
                  p.list.selEndId =
                    p.list.selEndId > 0 ? p.list.items[0].track_index : 0;
                  if (p.scrollbar.visible) on_mouse_wheel(1);
                } else if (mouse_y > p.list.y + p.list.h - cTrack.height * 0) {
                  p.list.selEndId =
                    p.list.selEndId < p.list.count - 1
                      ? p.list.items[p.list.items.length - 1].track_index
                      : p.list.count - 1;
                  if (p.scrollbar.visible) on_mouse_wheel(-1);
                }
                // set selection on items in the rect area drawn
                plman.SetPlaylistSelection(
                  p.list.playlist,
                  p.list.selAffected,
                  false,
                );
                p.list.selAffected.splice(0, p.list.selAffected.length);
                var deb =
                  p.list.selStartId <= p.list.selEndId
                    ? p.list.selStartId
                    : p.list.selEndId;
                var fin =
                  p.list.selStartId <= p.list.selEndId
...
oList = function (object_name, playlist) {
  ...
  this.items = [];
  this.groupby = [];
  this.totalGroupBy = window.GetProperty("SYSTEM.Groups.TotalGroupBy", 2);
  ...
  this.setItemColors = function () {
    this.text_colour_playing = blendColors(
      g_color_highlight,
      g_color_normal_bg,
      0.1,
    );
    this.text_colour_selected = blendColors(
      g_color_selected_txt,
      g_color_normal_bg,
      0.1,
    );
    ...
    if (this.totalGroupBy == 0) {
      ...
      for (var i = 0; i < 15; i++) {
        switch (i) {
          case 0:
            fields.push(
              new Array("Album Artist | Album | Disc", "Folder Structure"),
            );
            break;
          case 1:
            fields.push(
          ...
          case 10: // l1
            fields.push(
              new Array(
                "$if(%album%,%album% ['('Disc %discnumber%[ of %totaldiscs%]')'],$if(%length%,'Single(s)','Streams'))",
                "$directory(%path%,1)",
              ),
            );
            break;
          ...
          case 12: // l2
            fields.push(
              new Array("$if(%length%,%album artist%)", "$directory(%path%,2)"),
            );
            break;
          case 13: // r2
            fields.push(
              new Array("$if2(%genre%,'Other')", "$if2(%genre%,'Other')"),
            );
            break;
      ...
      for (var i = 0; i < 15; i++) {
        switch (i) {
          case 0:
            tmp = window.GetProperty(
              "SYSTEM.GroupBy.label",
              "Album Artist | Album | Disc^^Folder Structure",
            );
            break;
          case 1:
            tmp = window.GetProperty(
              "SYSTEM.GroupBy.tf",
              "%album artist%%album%%discnumber%^^$replace(%path%,%filename_ext%,)",
            );
            break;
          case 2:
            tmp = window.GetProperty(
              "SYSTEM.GroupBy.sortOrder",
              "%album artist% | $if(%album%,%date%,'9999') | %album% | %discnumber% | %tracknumber% | %title%^^%path%",
            );
            break;
          case 3:
            tmp = window.GetProperty("SYSTEM.GroupBy.ref", "Album^^Custom");
            break;
          case 4:
            tmp = window.GetProperty("SYSTEM.GroupBy.playlistFilter", "null^^null");
            break;
          case 5:
            tmp = window.GetProperty("SYSTEM.GroupBy.extraRows", "0^^0");
            break;
          case 6:
            tmp = window.GetProperty("SYSTEM.GroupBy.collapsedHeight", "2^^2");
            break;
          case 7:
            tmp = window.GetProperty("SYSTEM.GroupBy.expandedHeight", "3^^3");
            break;
          case 8:
            tmp = window.GetProperty("SYSTEM.GroupBy.showCover", "1^^1");
            break;
          case 9:
            tmp = window.GetProperty("SYSTEM.GroupBy.autoCollapse", "0^^0");
            break;
          case 10:
            tmp = window.GetProperty(
              "SYSTEM.GroupBy.l1",
              "$if(%album%,%album% ['('Disc %discnumber%[ of %totaldiscs%]')'],$if(%length%,'Single(s)','Streams'))^^$directory(%path%,1)",
            );
            break;
          case 11:
            tmp = window.GetProperty(
              "SYSTEM.GroupBy.r1",
              "$if(%date%,$year($replace(%date%,/,-,.,-)),'-')^^$if(%date%,$year($replace(%date%,/,-,.,-)),'-')",
            );
            break;
          case 12:
            tmp = window.GetProperty(
              "SYSTEM.GroupBy.l2",
              "$if(%length%,%album artist%)^^$directory(%path%,2)",
            );
            break;
          case 13:
            tmp = window.GetProperty(
              "SYSTEM.GroupBy.r2",
              "$if2(%genre%,'Other')^^$if2(%genre%,'Other')",
            );
            break;
          case 14:
            tmp = window.GetProperty(
              "SYSTEM.GroupBy.collapseGroupsByDefault",
              "0^^0",
            );
            break;
        }
  ...
  this.updateGroupsOnCollapse = function (group_id) {
    if (!this.groups[group_id].collapsed) {
      var delta =
        this.groups[group_id].count +
        this.groups[group_id].rowsToAdd +
        (cGroup.expanded_height - cGroup.collapsed_height);
      var fin = this.groups.length;
  ...
  this.updateGroupByPattern = function (pattern_idx) {
    ...
    properties.autocollapse =
      this.groupby[m].autoCollapse == "1" ? true : false;
    cGroup.extra_rows = Math.floor(this.groupby[m].extraRows);
    cGroup.default_collapsed_height = Math.floor(
      this.groupby[m].collapsedHeight,
    );
    ...
    properties.collapseGroupsByDefault =
      this.groupby[m].collapseGroupsByDefault == "1" ? true : false;
    // update max_w et max_h for cover loading and repaint in cache image handle functions
    cover.max_w =
      cGroup.default_collapsed_height > cGroup.default_expanded_height
        ? cGroup.default_collapsed_height * cTrack.height
        : cGroup.default_expanded_height * cTrack.height;
    cover.max_h =
      cGroup.default_collapsed_height > cGroup.default_expanded_height
        ? cGroup.default_collapsed_height * cTrack.height
        : cGroup.default_expanded_height * cTrack.height;
  ...
  this.init_groups = function (iscollapsed) {
    ...
    for (var i = 0; i < this.count; i++) {
      ...
      current = properties.showgroupheaders
        ? tf_group_key.EvalWithMetadb(handle)
        : handle.Path;
  ...
  this.showNowPlaying = function () {
    if (fb.IsPlaying) {
      if (plman.PlayingPlaylist != this.playlist) {
        ...
        plman.SetPlaylistFocusItem(
          this.playlist,
          this.nowplaying.PlaylistItemIndex,
        );
      } else {
        this.nowplaying = plman.GetPlayingItemLocation();
        // set focus on the now playing item
  ...
  this.getGroupIdfromTrackId = function (valeur) {
    ...
    while (deb <= fin) {
      ...
      if (
        valeur >= this.groups[mediane].start &&
        valeur < this.groups[mediane].start + this.groups[mediane].count
      ) {
        return mediane;
  ...
  this.getGroupIdFromRowId = function (valeur) {
    ...
    while (deb <= fin) {
      mediane = Math.floor((fin + deb) / 2);
      grp_height = this.groups[mediane].collapsed
        ? cGroup.collapsed_height
        : cGroup.expanded_height;
      grp_size = this.groups[mediane].collapsed
        ? grp_height
        : grp_height +
          this.groups[mediane].count +
          this.groups[mediane].rowsToAdd;
      if (
        valeur >= this.groups[mediane].totalPreviousRows &&
        valeur < this.groups[mediane].totalPreviousRows + grp_size
      ) {
        return mediane;
  ...
  this.getRowId = function (trackId) {
    ...
    if (this.groups[grp_id].collapsed) {
      // track hidden in the collapsed group so return = -1 or we return the row id of the group it belongs to ?
      //var row_index = -1;
      var row_index = this.groups[grp_id].totalPreviousRows + 1;
    } else {
      // group expanded so we can return a valid row_id for the track searched
      var row_index =
        this.groups[grp_id].totalPreviousRows +
        cGroup.expanded_height +
        (trackId - this.groups[grp_id].start);
    }
    return row_index;
  };
  ...
  this.getTrackId = function (rowId) {
    ...
    if (this.s_group_id >= 0) {
      this.s_group_height = this.groups[this.s_group_id].collapsed
        ? cGroup.collapsed_height
        : cGroup.expanded_height;
      if (this.groups[this.s_group_id].collapsed) {
        ...
        var a = rowId - this.groups[this.s_group_id].totalPreviousRows;
        if (a < this.s_group_height) {
          // row is in the group header
          this.s_groupheader_line_id = a;
          this.s_track_id = this.groups[this.s_group_id].start;
        } else {
          // row is a track
          ...
          this.s_track_id =
            a - this.s_group_height + this.groups[this.s_group_id].start;
          var track_index_in_group =
            this.s_track_id - this.groups[this.s_group_id].start;
          if (track_index_in_group >= this.groups[this.s_group_id].count) {
            // track is a copy of the last track of the group to fill the group with minimum track count in group feature!
            this.s_delta =
              track_index_in_group - this.groups[this.s_group_id].count + 1;
  ...
  this.scrollItems = function (delta, scrollstep) {
    ...
    this.setItems(false);
    p.scrollbar.setCursor(
      p.list.totalRowVisible,
      p.list.totalRows,
      p.list.offset,
    );
  ...
  this.setItems = function (forceFocus) {
    ...
    var m, n;
    if (forceFocus) {
      // from focus item centered in panel
      if (this.totalRows > this.totalRowVisible) {
        var i = this.getStartOffsetFromFocusId();
        if (this.totalRows - this.offset <= this.totalRowVisible) {
          var total_rows_to_draw =
            this.totalRows < this.totalRowVisible
              ? this.totalRows
              : this.totalRowVisible;
        } else {
          var total_rows_to_draw =
            this.totalRows < this.totalRowToLoad
              ? this.totalRows
              : this.totalRowToLoad;
        }
        ...
        while (i < this.offset + total_rows_to_draw) {
          this.getTrackId(i);
          if (this.s_groupheader_line_id >= 0) {
            // group header
            this.items.push(
              new oItem(
                this.playlist,
                row_index,
                1,
                this.handleList[this.s_track_id],
                this.s_track_id,
                this.s_group_id,
                0,
                this.s_group_height,
                this.s_groupheader_line_id,
                this.groups[this.s_group_id],
                0,
              ),
            );
            ...
            track_index_in_group =
              this.s_track_id -
              this.groups[this.s_group_id].start +
              this.s_delta;
            this.items.push(
              new oItem(
                this.playlist,
      ...
      // fill items from current offset
      if (this.totalRows > this.totalRowVisible) {
    ...
    if (this.totalRows <= this.totalRowVisible) {
      ...
      for (var i = 0; i < fin; i++) {
        if (this.items[i].group_index >= 0) {
          if (
            (this.items[i].type == 0 && this.items[i].empty_row_index == 0) ||
            this.groups[this.items[i].group_index].collapsed
          ) {
            if (this.groups[this.items[i].group_index].collapsed) {
              if (
                this.focusedTrackId >=
                  this.groups[this.items[i].group_index].start &&
                this.focusedTrackId <
                  this.groups[this.items[i].group_index].start +
                    this.groups[this.items[i].group_index].count
              ) {
                return true;
              }
            } else if (
              this.focusedTrackId == this.items[i].track_index &&
              this.items[i].row_index < this.totalRowVisible
            ) {
              return true;
            }
          }
  ...
  this.draw = function (gr) {
    ...
    this.state_queue_w = Math.round(
      gr.CalcTextWidth("00", g_font_queue_idx) + 1,
    );
    ...
    if (g_dragndrop_status && g_dragndrop_bottom) {
      var rowId = fin - 1;
      var item_height_row =
        this.items[rowId].type == 0 ? 1 : this.items[rowId].heightInRow;
      ...
      gr.FillSolidRect(
        rx + cover.w * 0,
        ry + item_height - Math.floor(cList.borderWidth / 2),
        rw - cover.w,
        cList.borderWidth,
        g_color_selected_bg,
      );
    ...
    if (this.drawRectSel) {
      ...
      if (this.selX <= mouse_x) {
        if (this.selY - this.selDeltaRows * cTrack.height <= mouse_y) {
          gr.FillSolidRect(
            this.selX,
            this.selY - this.selDeltaRows * cTrack.height,
            mouse_x - this.selX,
            mouse_y - (this.selY - this.selDeltaRows * cTrack.height),
            rectSelColor & 0x33ffffff,
          );
          gr.DrawRect(
            this.selX,
            this.selY - this.selDeltaRows * cTrack.height,
            mouse_x - this.selX - 1,
            mouse_y - (this.selY - this.selDeltaRows * cTrack.height) - 1,
            1.0,
            rectSelColor & 0x66ffffff,
          );
        } else {
          gr.FillSolidRect(
            this.selX,
            mouse_y,
            mouse_x - this.selX,
            this.selY - mouse_y - this.selDeltaRows * cTrack.height,
            rectSelColor & 0x33ffffff,
          );
          gr.DrawRect(
            this.selX,
            mouse_y,
            mouse_x - this.selX - 1,
            this.selY - mouse_y - this.selDeltaRows * cTrack.height - 1,
            1.0,
            rectSelColor & 0x66ffffff,
          );
        }
  ...
  this.isHoverObject = function (x, y) {
    return (
      x > this.x &&
      x < this.x + this.w - p.playlistManager.woffset &&
      y > this.y &&
      y < this.y + this.h
    );
  };
        ...
        if (this.ishover) {
          ...
          if (!cTouch.down) {
            if (!p.scrollbar.isHoverObject(x, y) && x < p.scrollbar.x) {
              // if not hover the scrollbar
              if (this.items.length > 0 && !this.item_clicked) {
                // and if click on an empty area of the playlist (after the last item)
                if (!properties.enableTouchControl) {
                  this.selX = x;
                  this.selY = y;
                  this.drawRectSel_click = true;
                  this.selStartId =
                    this.items[this.items.length - 1].track_index;
      ...
      case "drag_over":
        ...
        if (this.count > 0) {
          ...
          if (
            p.playlistManager.woffset == 0 ||
            (cPlaylistManager.visible &&
              x < p.playlistManager.x - p.playlistManager.woffset)
          ) {
            var rowId = fin - 1;
            var item_height_row =
              this.items[rowId].type == 0 ? 1 : this.items[rowId].heightInRow;
          ...
          if (dragndrop.moved || cPlaylistManager.drag_moved) {
            if (!cPlaylistManager.hscroll_timer) {
              ...
              if (p.playlistManager.woffset >= cPlaylistManager.width) {
                // if playlist manager panel opened
                ...
                var area_h =
                  p.playlistManager.h -
                  (cPlaylistManager.showStatusBar
                    ? cPlaylistManager.statusBarHeight
                    : 0);
                if (
                  x > p.playlistManager.x - p.playlistManager.woffset &&
                  x <
                    p.playlistManager.x -
                      p.playlistManager.woffset +
                      p.playlistManager.w
                ) {
                  if (
                    p.playlistManager.offset > 0 &&
                    y <
                      p.playlistManager.y +
                        cPlaylistManager.rowHeight +
                        inner_padding
                  ) {
                    if (
                      p.playlistManager.scrollbarWidth > 0 &&
                      !cPlaylistManager.vscroll_timer_loop
                    ) {
                      cPlaylistManager.vscroll_timer_loop = window.SetInterval(
                        function () {
                          //
                          var s = Math.abs(
                            mouse_y -
                          ...
                          if (!cPlaylistManager.vscroll_timer) {
                            cPlaylistManager.vscroll_timer = window.SetTimeout(
                              function () {
                                p.playlistManager.offset =
                                  p.playlistManager.offset > 0
                                    ? p.playlistManager.offset - 1
                                    : 0;
                                p.playlistManager.scrollbar.reSet(
                                  p.playlistManager.rowTotal,
                                  cPlaylistManager.rowHeight,
                                  p.playlistManager.offset,
                                );
                                full_repaint();
                    ...
                    p.playlistManager.offset <
                      p.playlistManager.rowTotal -
                        p.playlistManager.totalRows &&
                    y > p.playlistManager.y + area_h - inner_padding
                  ) {
                    if (
                      p.playlistManager.scrollbarWidth > 0 &&
                      !cPlaylistManager.vscroll_timer_loop
                      ...
                      cPlaylistManager.vscroll_timer_loop = window.SetInterval(
                        function () {
                          //
                          var s = Math.abs(
                            mouse_y - (p.playlistManager.y + area_h),
                          );
                          var h = Math.ceil(cPlaylistManager.rowHeight / 2);
                          if (s > h) s = h;
                          var t = h - s + 1;
                    ...
                    if (!cPlaylistManager.drag_move_timer) {
                      cPlaylistManager.drag_move_timer = window.SetTimeout(
                        function () {
                          if (
                            p.playlistManager.hoverId !=
                            p.playlistManager.hoverId_previous
                          ) {
                            full_repaint();
                            p.playlistManager.hoverId_previous =
                              p.playlistManager.hoverId;
                          ...
                          window.ClearInterval(
                            cPlaylistManager.drag_move_timer,
                          );
  ...
  this.incrementalSearch = function () {
    ...
    if (this.count > 1000) {
      albumartist = tf_albumartist.EvalWithMetadb(
        this.handleList[Math.floor(this.count / 2)],
      );
    ...
    if (pid >= 0) {
      ...
      // not found on "album artist" TAG, new search on "artist" TAG
      cList.inc_search_noresult = true;
      full_repaint();
    }

    cList.clear_incsearch_timer &&
      window.ClearTimeout(cList.clear_incsearch_timer);
  ...
  this.contextMenu = function (x, y, id, row_id) {
    ...
    _menu.AppendMenuItem(
      !plman.IsPlaylistLocked(this.playlist) &&
        fb.CheckClipboardContents(window.ID)
        ? MF_STRING
        : MF_GRAYED,
      1005,
      "Paste",
